      ...
      {isOpen && (
        <React.Fragment>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm bg-white dark:bg-[#1C1C1E] border border-slate-200 dark:border-white/10 rounded-3xl p-6 shadow-2xl z-50 flex flex-col gap-6"
          >
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                <span className="font-semibold text-sm text-slate-700 dark:text-zinc-200">Erscheinungsbild</span>
                
                {/* Theme Toggle (Segmented Control Style) */}
                <button 
                  onClick={toggleTheme}
                  className="relative flex items-center w-[120px] h-10 bg-slate-200 dark:bg-black/50 rounded-full p-1"
                >
                  <div className="flex-1 text-center z-10 text-[11px] font-bold text-slate-700 dark:text-zinc-400">Light</div>
                  <div className="flex-1 text-center z-10 text-[11px] font-bold text-slate-500 dark:text-zinc-100">Dark</div>
